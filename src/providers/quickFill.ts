import { Range, TextDocumentChangeEvent, TextEditor, window } from "vscode";
import { Dependency, Item } from "../core/interfaces";
import { fetchedDepsMap, getFetchedDependency } from "../core/listener";
import { RE_VERSION, findPackageAndVersion } from "../toml/parser";
...
export async function quickFillListener(e: TextDocumentChangeEvent) {
  ...
  if (
    ...
    let pypackage: string;
    let version: string;
    if (versionMatch) {
      pypackage = versionMatch[1];
      version = versionMatch[7] ?? versionMatch[5];
    } else {
      const match = findPackageAndVersion(editor.document, line);
      if (!match) return;

...
import { RE_VERSION, RE_EXTRAS, findPackage, findPackageAndVersion } from "../toml/parser";
...
export class VersionCompletions implements CompletionItemProvider {
  provideCompletionItems(
    ...
    if (match) {
      const pypackage = match[1] === "version" ? findPackage(document, position.line) : match[1];
      if (!pypackage) return;

      const version = match[7] ?? match[5];

      const fetchedDep = getFetchedDependency(document, pypackage, position);
      ...
      if (version.trim().length !== 0) {
        const filterVersion = version
          .substring(0, versionStart - position.character)
          .toLowerCase();
...
export class FeaturesCompletions implements CompletionItemProvider {
  provideCompletionItems(
    ...
    const featuresMatch = line.text.match(RE_EXTRAS);
    if (featuresMatch) {
      let pypackage;
      let version;
      const versionMatch = line.text.match(RE_VERSION);
      if (versionMatch) {
        pypackage = versionMatch[1];
        version = versionMatch[7] ?? versionMatch[5];
      } else {
        const match = findPackageAndVersion(document, position.line);
        if (!match) return;
        [pypackage, version] = match;
      }

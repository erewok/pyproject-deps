...
export const RE_EXTRAS = /^[ \t]*(?<!#)((?:[\S]+?[ \t]*=[ \t]*.*?{.*?)?extras[ \t]*=[ \t]*\[[ \t]*)(.+?)[ \t]*\]/;
...
export function findPackageAndVersion(
  ...
  while (!pypackage && --i >= 0) {
    ...
    if (!match) {
      ...
      pypackage = match[1].match(RE_TABLE_HEADER_DEPENDENCY)?.[1];
    }
  }
  ...
  if (pypackage && version) {
    return [pypackage, version];
  }
}

/**
 * Finds all version items with a flat pypackage=version pair.
...
/**
 * Filters all dependency related items with a flat pypackage=version match.
 * @param items
 */
